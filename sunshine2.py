...
def solar_cell_model(V, params, I_min, I_max):
    ...
    #print("当前I_ph的值为:", I_ph)

    #I_ph=15
    #I0 = 1e-10  # 手动设置合理的反向饱和电流
...
def objective_function(params, V, I_meas, I_min, I_max):
    ...
    # 新增：打印模型计算的关键信息（仅前5个点，避免刷屏）
    #print("\n===== 模型计算结果（前5点） =====")
    #print(f"实测电流：{I_meas[:5]}")
    #print(f"模拟电流：{I_sim[:5]}")
    ...
    def _teaching_phase(self, best_params, explore_ratio):
        ...
        for params in self.population:
            ...
            if np.random.uniform(0, 1) < explore_ratio:
                # 合理的探索范围（避免参数溢出）
                new_params = [
                    params[0] * np.random.uniform(0.5, 1.5),
                    params[1] * np.random.uniform(0.5, 1.5),
                    params[2] * np.random.uniform(0.8, 1.2),
                    params[3] * np.random.uniform(0.5, 1.5),
                    params[4] * np.random.uniform(0.8, 1.2)
                ]
            else:
                # 自适应向老师学习
    ...
    def train(self):
        ...
        for iter in range(self.max_iter):
            ...
            if best_loss < global_best_loss:
                ...
                global_best_params = best_params.copy()  # 修复：显式拷贝参数
